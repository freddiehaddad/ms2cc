        ...
        if path.is_file() {
            if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
                if parser::should_process_file_extension(ext, &config.file_extensions) {
                    processed_files.push(path.file_name().unwrap().to_string_lossy().to_string());
                }
            }
        }
    }
...
fn test_path_normalization() {
    ...
    for (input, expected) in &test_cases {
        ...
        if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
            let config = Config::default();
            assert!(parser::should_process_file_extension(ext, &config.file_extensions));
        }
    }
}
...
fn test_edge_cases_with_file_paths() {
    ...
    assert!(parser::should_process_file_extension("CPP", &config.file_extensions));
    assert!(parser::should_process_file_extension("Hpp", &config.file_extensions));
