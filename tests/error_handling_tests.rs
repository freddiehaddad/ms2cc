...
use ms2cc::{parser, compile_commands, Config};
...
fn test_malformed_input_handling() {
    ...
    for case in &malformed_cases {
        ...
        assert!(cleaned.len() <= case.len(), "Cleanup should not increase length");
    }
}
...
fn test_boundary_conditions() {
    ...
    let mixed = format!("{}cl.exe{} /c{} main.cpp{}", "\"".repeat(100), "\"".repeat(100), "\"".repeat(100), "\"".repeat(100));
    ...
    let edge_case_dirs = ["", ".", "..", "very_long_directory_name_that_might_cause_issues", "dir-with-dashes", "dir.with.dots"];
    
    for dir in &edge_case_dirs {
        let result = parser::should_exclude_directory(dir, &config.exclude_directories);
        // Should not panic and return a boolean
        assert!(result == true || result == false);
    }
    
    // Test file extension edge cases
    let edge_case_exts = ["", "c", "C", "cPP", "h++", "very_long_extension", "1", "123"];
    
    for ext in &edge_case_exts {
        let result = parser::should_process_file_extension(ext, &config.file_extensions);
        // Should not panic and return a boolean
        assert!(result == true || result == false);
    }
}
...
fn test_unicode_and_special_characters() {
    ...
    for case in &unicode_cases {
        ...
        let cleaned = parser::cleanup_line(case);
        assert_eq!(cleaned, *case, "Unicode strings without quotes should be unchanged");
    }
    ...
    for case in &special_chars {
        let tokens = parser::tokenize_compile_command(case);
        assert!(tokens.len() >= 3, "Should handle special characters: {}", case);
...
fn test_case_sensitivity() {
    ...
    for (ext, should_match) in &case_variants {
        let result = parser::should_process_file_extension(ext, &config.file_extensions);
        assert_eq!(result, *should_match, "Case sensitivity test failed for: {}", ext);
    }
    ...
    for (dir, should_exclude) in &dir_variants {
        let result = parser::should_exclude_directory(dir, &config.exclude_directories);
        assert_eq!(result, *should_exclude, "Directory exclusion test failed for: {}", dir);
    }
}
    ...
    let handles: Vec<_> = (0..10).map(|i| {
        ...
        thread::spawn(move || {
            ...
            for j in 0..100 {
                ...
                let _ = parser::should_process_file_extension(&test_ext, &config.file_extensions);
                
                let test_dir = format!("dir{}", (i * 100 + j) % 3);
                let _ = parser::should_exclude_directory(&test_dir, &config.exclude_directories);
                
                let test_line = format!("cl.exe /c test{}.cpp", i * 100 + j);
                let _ = parser::tokenize_compile_command(&test_line);
                let _ = parser::cleanup_line(&test_line);
                let _ = parser::ends_with_cpp_source_file(&test_line, &config.file_extensions);
            }
        })
    }).collect();
...
fn test_memory_usage_patterns() {
    ...
    for _ in 0..1000 {
        let large_string = "cl.exe ".to_owned() + &"/D DEFINE=1 ".repeat(100) + "main.cpp";
...
fn test_compile_command_creation_errors() {
    ...
    for path_str in &error_cases {
        ...
        let args = vec!["cl.exe".to_string(), "/c".to_string(), path_str.to_string()];
        ...
        if path_str.len() <= 3 {  // Root paths like "/" or "C:\"
...
fn test_fo_argument_edge_cases() {
    ...
    for (fo_arg, should_succeed) in &fo_cases {
        ...
        if *should_succeed {
            assert!(result.is_ok(), "/Fo parsing should succeed for: {}", fo_arg);
        } else {
