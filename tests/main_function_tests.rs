...
fn test_msbuild_log_parsing_realistic() {
    ...
    writeln!(file, "Copyright (C) Microsoft Corporation. All rights reserved.").unwrap();
    writeln!(file, "").unwrap();
    ...
    writeln!(file, "").unwrap();
    writeln!(file, "  cl.exe /c /Zi /nologo /W3 /WX- /diagnostics:column /Od /Ob0 /D WIN32").unwrap();
    ...
    for line in content.lines() {
        ...
        if line_lower.contains(&config.compiler_executable.to_lowercase()) {
            if parser::ends_with_cpp_source_file(&line_lower, &config.file_extensions) {
...
fn test_large_directory_simulation() {
    ...
    fn visit_directory(dir: &Path, config: &Config, processed: &mut usize, skipped: &mut usize) {
        for entry in fs::read_dir(dir).unwrap() {
            ...
            if path.is_dir() {
                if let Some(dir_name) = path.file_name().and_then(|n| n.to_str()) {
                    if !parser::should_exclude_directory(dir_name, &config.exclude_directories) {
                        visit_directory(&path, config, processed, skipped);
                    }
                }
            } else if path.is_file() {
                if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
                    if parser::should_process_file_extension(ext, &config.file_extensions) {
                        *processed += 1;
                    } else {
                        *skipped += 1;
                    }
    ...
    visit_directory(temp_dir.path(), &config, &mut processed_files, &mut skipped_files);
...
fn test_error_conditions_in_parsing() {
    ...
    let test_cases = [
        ...
        // Very long line
        (&format!("cl.exe {} main.cpp", "/D VERY_LONG_DEFINE=1".repeat(100)), true),
    ];
    
    for (line, should_match) in &test_cases {
        let contains_compiler = line.to_lowercase().contains(&config.compiler_executable.to_lowercase());
        let ends_with_cpp = parser::ends_with_cpp_source_file(line, &config.file_extensions);
        
        if *should_match {
            assert!(contains_compiler && ends_with_cpp, "Line should match: {}", line);
        } else {
            assert!(!(contains_compiler && ends_with_cpp), "Line should not match: {}", line);
        }
    }
}
...
fn test_tokenization_edge_cases() {
    let test_cases = [
        ...
        (&format!("cl.exe {} main.cpp", (0..50).map(|_| "/DVERY_LONG_DEFINE=\"test value\"").collect::<Vec<_>>().join(" ")), {
...
fn test_quote_cleanup_edge_cases() {
    let test_cases = [
        ...
        ("cl.exe /c \"path with spaces/main.cpp\"", "cl.exe /c path with spaces/main.cpp"),
        // Only quotes
        ("\"\"\"", ""),
...
fn test_path_reconstruction_scenarios() {
    ...
    let test_cases = [
        // Absolute path
        (vec!["cl.exe".to_string(), "/c".to_string(), cpp_file.to_string_lossy().to_string()], true),
        // Relative path
        (vec!["cl.exe".to_string(), "/c".to_string(), "main.cpp".to_string()], false), // No file map
        // With /Fo argument
        (vec!["cl.exe".to_string(), "/c".to_string(), format!("/Fo{}", src_dir.to_string_lossy()), "main.cpp".to_string()], true),
        // Invalid /Fo argument
        (vec!["cl.exe".to_string(), "/c".to_string(), "/FoNonExistent/".to_string(), "main.cpp".to_string()], false),
        // Empty arguments
        (vec![], false),
        // No source file
    ...
    for (args, _should_succeed) in &test_cases {
        ...
        if last_arg.ends_with(".cpp") {
            assert!(filename_result.is_ok(), "Should extract filename from: {}", last_arg);
        } else {
        ...
        let handles: Vec<_> = (0..4).map(|_| {
            s.spawn(move || {
                ...
                for ext in &["cpp", "h", "c", "txt", "py"] {
                    let _ = parser::should_process_file_extension(ext, &config_ref.file_extensions);
                }
                
                for dir in &[".git", "src", "target", "build"] {
                    let _ = parser::should_exclude_directory(dir, &config_ref.exclude_directories);
                }
            })
        }).collect();
...
fn test_memory_efficiency() {
    ...
    let large_line = "cl.exe ".to_owned() + &"/DTEST=1 ".repeat(1000) + "main.cpp";
